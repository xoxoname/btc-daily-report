...
예시:
- "오늘 $200 수익을 냈네요! 꾸준한 수익이 복리의 힘을 만듭니다. 욕심내지 말고 현재 페이스를 유지하세요. 💪"
- "지금 $150 손실이 있지만 너무 걱정하지 마세요. 지난주 $900 수익을 생각하면 일시적인 조정일 뿐입니다. 🌱"
"""
    ...
    def _big_win_messages(self, today_pnl: float, total_equity: float, weekly_total: float) -> str:
        """큰 수익 시 메시지"""
        patterns = [
            f'"오늘 ${today_pnl:.0f} 벌었네요! 대단합니다! 하지만 시장은 항상 겸손해야 한다는 것을 기억하세요. 수익을 지키는 것이 버는 것보다 중요합니다. 🎯"',
            f'"와! ${today_pnl:.0f} 수익이라니 정말 잘했어요! 이제 총 자산이 ${total_equity:,.0f}이 되었네요. 하지만 과욕은 금물, 리스크 관리를 잊지 마세요. 💪"',
            f'"${today_pnl:.0f} 수익 축하해요! 편의점 알바 {int(today_pnl/15):.0f}시간 번 셈이네요. 이 기세를 유지하되 신중함도 함께 가져가세요. 🚀"',
            f'"오늘의 ${today_pnl:.0f} 달러는 당신의 실력입니다. 7일간 ${weekly_total:.0f} 벌었다는 건 운이 아니에요. 계속 시스템을 믿고 따르세요. ⭐"'
        ]
    ...
    def _steady_profit_messages(self, today_pnl: float, weekly_avg: float, total_equity: float) -> str:
        """꾸준한 수익 시 메시지"""
        patterns = [
            f'"오늘도 ${today_pnl:.0f} 수익, 정말 꾸준하네요! 일주일 평균 ${weekly_avg:.0f}/일이면 월 ${weekly_avg*30:.0f} 예상이에요. 복리의 마법이 시작됐네요! 📈"',
            f'"${today_pnl:.0f} 벌었군요! 작아 보여도 이런 꾸준함이 ${total_equity:,.0f}을 만들었어요. 천천히 그러나 확실하게 가는 거예요. 🌱"',
            f'"하루 ${today_pnl:.0f} 달러, 훌륭해요! 커피값이라도 매일 벌면 한 달이면 ${today_pnl*30:.0f}이에요. 이 페이스 유지하세요! 🎯"',
            f'"오늘도 ${today_pnl:.0f} 플러스! 급하게 큰 돈 벌려고 하지 마세요. 지금처럼만 하면 충분해요. 꾸준함이 최고의 전략입니다. ✨"'
        ]
        return random.choice(patterns)
    ...
    def _small_loss_messages(self, unrealized_pnl: float, weekly_total: float) -> str:
        """소폭 손실 시 메시지"""
        patterns = [
            f'"지금 ${abs(unrealized_pnl):.0f} 마이너스지만 괜찮아요. 최근 7일간 ${weekly_total:.0f} 벌었잖아요. 일시적인 조정일 뿐이에요. 차분하게 대응하세요. 🧘‍♂️"',
            f'"${abs(unrealized_pnl):.0f} 손실이 있네요. 하지만 손실도 트레이딩의 일부예요. 중요한 건 감정적으로 대응하지 않는 거예요. 계획을 따르세요. 💪"',
            f'"마이너스 ${abs(unrealized_pnl):.0f}이지만 너무 신경 쓰지 마세요. 지난주 ${weekly_total:.0f} 수익을 보세요. 당신은 할 수 있어요. 이번 경험에서 배우세요. 🌱"',
            f'"${abs(unrealized_pnl):.0f} 손실 중이네요. 모든 트레이더가 겪는 일이에요. 손실을 최소화하고 다음 기회를 준비하는 게 프로의 자세입니다. 🎯"'
        ]
        return random.choice(patterns)
    ...
    def _big_loss_messages(self, unrealized_pnl: float, total_equity: float) -> str:
        """큰 손실 시 메시지"""
        patterns = [
            f'"${abs(unrealized_pnl):.0f} 손실이 크긴 하네요. 하지만 아직 ${total_equity:,.0f}이 있어요. 지금은 멈추고 다시 계획을 세울 때예요. 감정적 거래는 금물입니다. 🛡️"',
            f'"${abs(unrealized_pnl):.0f} 손실... 힘들겠지만 이것도 성장의 과정이에요. 지금 중요한 건 더 큰 손실을 막는 거예요. 잠시 쉬어가도 괜찮아요. 🤝"',
            f'"${abs(unrealized_pnl):.0f} 마이너스는 아프지만 끝이 아니에요. 총 자산의 {abs(unrealized_pnl)/total_equity*100:.0f}%일 뿐이에요. 손절의 용기를 가지세요. 💪"',
            f'"지금 ${abs(unrealized_pnl):.0f} 손실이지만 포기하지 마세요. 시장은 항상 기회를 줍니다. 하지만 먼저 감정을 추스르고 냉정해지세요. 🎯"'
        ]
        return random.choice(patterns)
    ...
    def _break_even_messages(self, total_equity: float, weekly_total: float) -> str:
        """손익 균형 시 메시지"""
        patterns = [
            f'"오늘은 큰 변화가 없네요. 때로는 기다리는 것도 실력이에요. ${total_equity:,.0f}을 지킨 것만으로도 잘한 거예요. 기회는 곧 옵니다. ⚖️"',
            f'"손익 제로, 나쁘지 않아요! 잃지 않은 것도 이긴 거예요. 최근 7일 ${weekly_total:.0f} 수익도 있고요. 인내심을 가지세요. 🎯"',
            f'"오늘은 평온한 날이네요. 매일 수익을 낼 필요는 없어요. 지금처럼 차분히 기회를 기다리는 것도 전략입니다. 📊"',
            f'"변동이 없는 날도 있죠. 거래하지 않는 것이 최고의 거래일 때도 있어요. 자산 ${total_equity:,.0f}을 잘 지키고 있네요. ✨"'
        ]
        return random.choice(patterns)
    ...
    def _general_messages(self, total_equity: float, weekly_total: float) -> str:
        """일반적인 메시지"""
        patterns = [
            f'"현재 자산 ${total_equity:,.0f}, 잘 관리하고 있네요. 트레이딩은 마라톤이에요. 꾸준히 가는 게 중요합니다. 화이팅! 📈"',
            f'"최근 7일간 ${weekly_total:.0f} 수익, 나쁘지 않아요! 감정보다는 전략을 믿고 따르세요. 그게 성공의 비결이에요. 💪"',
            f'"자산 ${total_equity:,.0f}을 운용 중이시네요. 매일의 작은 결정이 큰 결과를 만들어요. 오늘도 현명한 선택을 하세요! 🌟"',
