...
class MirrorTradingUtils:
    ...
    async def calculate_dynamic_margin_ratio_with_multiplier(self, size: float, trigger_price: float, 
        ...
        try:
            ...
            if validated_ratio != ratio_multiplier:
                self.logger.warning(f"비율 조정됨: {ratio_multiplier} → {validated_ratio}")
                ratio_multiplier = validated_ratio
            
            self.logger.info(f"비율 적용 마진 계산: size={size}, ratio={ratio_multiplier}x")
            ...
            if adjusted_margin_ratio <= 0:
                return {
                    'success': False,
                    'error': f'조정된 마진 비율이 0이거나 음수: {adjusted_margin_ratio:.4f}'
                }
            ...
            self.logger.error(f"비율 적용 마진 계산 실패: {e}")
            return {
                'success': False,
                'error': str(e)
            }
    ...
    def validate_ratio_multiplier(self, ratio_multiplier: float) -> float:
        try:
            ...
            if ratio_multiplier < self.MIN_RATIO_MULTIPLIER:
                self.logger.warning(f"비율이 너무 작음 ({ratio_multiplier}), 최소값 사용: {self.MIN_RATIO_MULTIPLIER}")
                return self.MIN_RATIO_MULTIPLIER
            
            if ratio_multiplier > self.MAX_RATIO_MULTIPLIER:
                self.logger.warning(f"비율이 너무 큼 ({ratio_multiplier}), 최대값 사용: {self.MAX_RATIO_MULTIPLIER}")
                return self.MAX_RATIO_MULTIPLIER
            
            # 위험도 경고
            if ratio_multiplier > 5.0:
                self.logger.warning(f"매우 높은 비율 ({ratio_multiplier}x). 리스크 관리 필요.")
            elif ratio_multiplier < 0.5:
                self.logger.info(f"보수적 비율 ({ratio_multiplier}x). 안전한 설정.")
            
            return ratio_multiplier
            
        except (ValueError, TypeError):
            self.logger.error(f"비율 변환 실패 ({ratio_multiplier}), 기본값 사용: {self.DEFAULT_RATIO_MULTIPLIER}")
            return self.DEFAULT_RATIO_MULTIPLIER
    ...
    def get_ratio_multiplier_description(self, ratio_multiplier: float) -> str:
        try:
            ...
            self.logger.error(f"비율 설명 생성 실패: {e}")
            return "비율 정보 없음"
    ...
    def analyze_ratio_multiplier_effect(self, ratio_multiplier: float, base_ratio: float, adjusted_ratio: float) -> Dict:
        try:
            ...
            self.logger.error(f"비율 효과 분석 실패: {e}")
            return {
                'multiplier': ratio_multiplier,
                'description': "분석 실패",
                'impact': "알 수 없음",
    ...
    async def extract_bitget_leverage_enhanced(self, order_data: Dict = None, position_data: Dict = None, account_data: Dict = None) -> int:
        try:
            ...
            try:
                    ...
                    if account_leverage:
                        try:
                            ...
                            if self.MIN_LEVERAGE <= lev_value <= self.MAX_LEVERAGE:
                                ...
                                self.logger.info(f"신규 계정에서 레버리지 추출: {extracted_leverage}x ({source})")
                ...
                self.logger.warning(f"신규 계정 조회 실패: {e}")
                        ...
                        if pos_leverage:
                            try:
                                ...
                                if self.MIN_LEVERAGE <= lev_value <= self.MAX_LEVERAGE:
                                    ...
                                    self.logger.info(f"라이브 포지션에서 레버리지 추출: {extracted_leverage}x ({source})")
                                    return extracted_leverage
                            except (ValueError, TypeError):
                                continue
            except Exception as e:
                self.logger.warning(f"라이브 포지션 조회 실패: {e}")
        ...
        try:
            ...
            # 2. 명시적 클로즈 키워드
                ...
                for keyword in self.CLOSE_ORDER_KEYWORDS:
                    if keyword in side:
                        ...
                        self.logger.info(f"side 키워드로 클로즈 주문 확인: '{side}' contains '{keyword}'")
                        break
            ...
            # 3. TP/SL 전용 주문 타입
                ...
                for tp_sl_type in self.TP_SL_ONLY_ORDER_TYPES:
                    if tp_sl_type in order_type:
                        ...
                        self.logger.info(f"TP/SL 타입으로 클로즈 주문 확인: '{order_type}' contains '{tp_sl_type}'")
                        break
            
            # 4. TP/SL이 있는 주문은 오픈 주문으로 처리 (잘못된 분류 방지)
            if not is_close_order:
                ...
                if tp_price or sl_price:
                    ...
                    self.logger.info(f"TP/SL 설정이 있는 오픈 주문 (TP={tp_price}, SL={sl_price})")
                    self.logger.info(f"       → TP/SL과 함께 새 포지션 생성")
            ...
            if not is_close_order:
                ...
                self.logger.info(f"오픈 주문으로 확인 (새 포지션 생성)")
            ...
            if is_close_order:
                ...
                if 'close_long' in side or 'exit_long' in side:
                    order_direction = 'sell'  # 롱 포지션을 매도로 클로즈
                    position_side = 'long'
                elif 'close_short' in side or 'exit_short' in side:
                    order_direction = 'buy'   # 숏 포지션을 매수로 클로즈
                    position_side = 'short'
                    ...
                    try:
                        ...
                        if active_positions:
                            ...
                            if current_side == 'long':
                                order_direction = 'sell'
                                position_side = 'long'
                                self.logger.info(f"현재 롱 포지션 기반: 클로즈를 위해 매도")
                            elif current_side == 'short':
                                order_direction = 'buy'
                                position_side = 'short'
                                self.logger.info(f"현재 숏 포지션 기반: 클로즈를 위해 매수")
                            else:
                                order_direction = 'sell'
                                position_side = 'long'
                        else:
            ...
            self.logger.error(f"강화된 클로즈 주문 세부 사항 결정 실패: {e}")
            return {
                'is_close_order': False,
                'order_direction': 'buy',
                'position_side': 'long',
    ...
    async def extract_gate_order_details(self, gate_order: Dict) -> Optional[Dict]:
        try:
            ...
            self.logger.error(f"게이트 주문 세부 사항 추출 실패: {e}")
            return None
    ...
    async def generate_multiple_order_hashes(self, order_details: Dict) -> List[str]:
        try:
            ...
            if abs_size > 0:
                ...
                self.logger.debug(f"크기가 0, 가격 기반 해시만 생성 - trigger_price: {trigger_price}")
    ...
    async def adjust_price_for_gate(self, price: float, bitget_current_price: float = 0, 
        ...
        try:
            ...
            if (bitget_current_price > 0 and gate_current_price > 0):
                ...
                # 비정상적인 가격 차이에 대한 매우 높은 임계값 (10000+ 달러)
                if price_diff_abs > self.ABNORMAL_PRICE_DIFF_THRESHOLD:
                    self.logger.info(f"매우 큰 가격 차이지만 처리 계속 (${price_diff_abs:.2f})")
                    return price  # 조정 없이 원래 가격 사용
                
                # 가격 차이와 관계없이 항상 처리하되, 조정은 선택적으로 적용
                if (self.PRICE_ADJUSTMENT_ENABLED and 
                    ...
                    # 조정 크기 검증 (매우 관대함: 최대 50% 조정 허용)
                    adjustment_percent = abs(adjusted_price - price) / price * 100
                    
                    if adjustment_percent <= 50.0:  # 10% → 50% 훨씬 더 관대하게
                        self.logger.info(f"가격 조정: ${price:.2f} → ${adjusted_price:.2f} (차이: ${price_diff_abs:.2f})")
                        return adjusted_price
                    else:
                        self.logger.info(f"큰 조정이지만 원래 가격으로 계속 진행 ({adjustment_percent:.1f}%)")
                        return price  # 조정 없이 처리 계속
                else:
                    return price
            elif bitget_current_price <= 0 or gate_current_price <= 0:
                self.logger.debug("가격 조회 실패했지만 처리 계속")
                return price
            
            return price
            
        except Exception as e:
            self.logger.error(f"가격 조정 실패했지만 처리 계속: {e}")
            return price or 0
    
    async def validate_trigger_price(self, trigger_price: float, side: str, current_price: float = 0) -> Tuple[bool, str]:
        try:
            if trigger_price is None or trigger_price <= 0:
                return False, "트리거 가격이 None이거나 <= 0"
            
            if current_price <= 0:
                self.logger.info("현재 가격 조회 실패했지만 모든 트리거 가격 허용")
                return True, "현재 가격 조회 실패했지만 허용됨"
            ...
            # 비정상적인 가격 차이 감지를 위한 매우 높은 임계값 (10000달러)
            if price_diff_abs > self.ABNORMAL_PRICE_DIFF_THRESHOLD:
            ...
            self.logger.error(f"트리거 가격 검증 실패했지만 모든 가격 허용: {e}")
            return True, f"검증 오류지만 모든 가격 허용: {str(e)[:100]}"
    ...
    async def calculate_gate_order_size_for_close_order_enhanced(self, current_gate_position_size: int, 
        ...
        try:
            ...
            if current_gate_position_size == 0:
                self.logger.warning(f"현재 포지션이 0이지만 클로즈 주문 생성 강제")
            ...
            # 비트겟 클로즈 주문에서 부분 클로즈 비율 계산
            bitget_size = float(bitget_order.get('size', 0))
            
            # 비트겟 현재 포지션을 조회하여 부분 클로즈 비율 계산
            try:
                bitget_positions = await self.bitget.get_positions(self.SYMBOL)
                ...
                if bitget_current_position:
                    ...
                    if bitget_position_size > 0:
                        ...
                        self.logger.info(f"부분 클로즈 비율: {close_ratio*100:.1f}% (비트겟 포지션: {bitget_position_size}, 클로즈 크기: {bitget_size})")
                    else:
                        close_ratio = 1.0
                        self.logger.warning(f"비트겟 포지션 크기가 0, 전체 클로즈로 처리")
                else:
                    # 해당하는 비트겟 포지션을 찾을 수 없음, 전체 클로즈 사용
                    close_ratio = 1.0
                    self.logger.warning(f"해당하는 비트겟 포지션을 찾을 수 없음, 전체 클로즈로 처리")
                    
            except Exception as e:
                # 비트겟 포지션 조회 실패, 전체 클로즈 사용
                close_ratio = 1.0
                self.logger.error(f"비트겟 포지션 조회 실패, 전체 클로즈로 처리: {e}")
    ...
    async def calculate_gate_order_size_fixed(self, side: str, base_size: int, is_close_order: bool = False) -> Tuple[int, bool]:
        try:
            ...
            if is_close_order or 'close' in side_lower:
                ...
                # 클로즈 주문: 포지션을 클로즈하는 방향으로 주문
                if 'close_long' in side_lower or side_lower == 'close long':
                    # 롱 포지션 클로즈 → 매도 주문 (음수)
                    gate_size = -abs(base_size)
                    self.logger.info(f"클로즈 롱: 롱 포지션 클로즈 → 게이트 매도 주문 (음수: {gate_size})")
                    
                elif 'close_short' in side_lower or side_lower == 'close short':
                    # 숏 포지션 클로즈 → 매수 주문 (양수)
                    gate_size = abs(base_size)
                    self.logger.info(f"클로즈 숏: 숏 포지션 클로즈 → 게이트 매수 주문 (양수: {gate_size})")
                    
                elif 'sell' in side_lower and 'buy' not in side_lower:
                    # 매도로 클로즈 → 롱 포지션 클로징
                    gate_size = -abs(base_size)
                    self.logger.info(f"클로즈 매도: 롱 포지션 클로즈 → 게이트 매도 주문 (음수: {gate_size})")
                    
                elif 'buy' in side_lower and 'sell' not in side_lower:
                    # 매수로 클로즈 → 숏 포지션 클로징
                    gate_size = abs(base_size)
                    self.logger.info(f"클로즈 매수: 숏 포지션 클로즈 → 게이트 매수 주문 (양수: {gate_size})")
            ...
            # 오픈 주문 처리
                ...
                if 'open_long' in side_lower or ('buy' in side_lower and 'sell' not in side_lower):
                    ...
                    # 기타 오픈 주문 - 원래 크기 유지
                    gate_size = base_size
                    self.logger.warning(f"알 수 없는 오픈 주문 타입: {side}, 원래 크기 유지: {gate_size}")
    ...
    async def get_current_gate_position_size(self, gate_mirror_client, position_side: str = None) -> Tuple[int, str]:
        try:
            ...
            if not gate_positions:
                self.logger.info("게이트 포지션을 찾을 수 없음")
                return 0, 'none'
    ...
    async def validate_close_order_against_position(self, close_order_details: Dict, 
        ...
        try:
            ...
            self.logger.error(f"클로즈 주문 검증 실패했지만 허용: {e}")
            return True, f"검증 오류지만 클로즈 주문 허용: {str(e)}"
    ...
    async def get_price_difference_info(self, bitget_price: float, gate_price: float) -> Dict:
        try:
            ...
            self.logger.error(f"가격 차이 정보 계산 실패: {e}")
            return {
                'price_diff_abs': 0,
                'price_diff_percent': 0,
                'exceeds_threshold': False,
                'status': 'error',
                'is_abnormal': False,
                'should_process': True  # 오류가 있어도 처리 진행
            }
