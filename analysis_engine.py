...
지정된 한국어 형식으로 정확히 응답해주세요.

분석 시 고려사항:
1. 기술적 분석: RSI, 볼린저밴드, 이동평균, 지지/저항선
2. 심리적 분석: 펀딩비, 미결제약정, 공포탐욕지수
3. 구조적 분석: 온체인 데이터, 대량 거래
4. 리스크 관리: 레버리지, 청산 위험
...
분석 유형: {analysis_type}
요청사항: 주어진 데이터를 바탕으로 비트코인 선물 시장 분석을 수행하고, 
지정된 한국어 텔레그램 메시지 형식으로 정확히 작성해주세요.
"""
    ...
    async def _calculate_profit_info(self, market_data: Dict) -> Dict:
        ...
        try:
            ...
            if positions:
                ...
                # 청산가 계산 (근사치)
                margin = total_equity / leverage if leverage > 0 else total_equity
                liquidation_distance = (current_price - entry_price) / current_price * 100 if current_price > 0 else 0
...
- 미실현 손익: ${unrealized_pnl:.1f}"""
            ...
            # 수익률 계산 (가정: 초기 자산 기준)
            initial_balance = 2000  # 임시값, 실제로는 DB에서 관리해야 함
