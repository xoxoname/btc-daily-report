...
class MirrorTradingSystem:
    def __init__(self, config, bitget_client, gate_client, telegram_bot):
        ...
        # 체결된 주문 추적 강화 - 취소와 정확한 구분
        self.filled_order_tracking_enabled = True
        self.filled_order_check_interval = 3  # 3초마다 체결된 주문 확인 (더 빠르게)
        self.last_filled_order_check = datetime.min
        self.recently_filled_orders = set()  # 최근 체결된 주문 ID 추적
        self.filled_order_cache_time = 600  # 10분간 캐시
        ...
        self.warning_counters = {
            ...
            'margin_mode_warning': 0,
            'filled_order_detection': 0  # 새로 추가: 체결 주문 감지 관련
        }
        ...
        self.position_wait_timeout = 60
        
        # 시세 조회 실패 관리 강화
        self.last_valid_bitget_price = 0.0
        self.last_valid_gate_price = 0.0
        self.bitget_price_failures = 0
        self.gate_price_failures = 0
        self.max_price_failures = 10
        
        # 예약 주문 동기화 강화 설정 - 개선된 버전
        self.order_sync_enabled = True
        self.order_sync_interval = 45  # 45초마다 동기화 체크
        self.last_order_sync_time = datetime.min
        ...
        self.logger.info(f"   - 체결 주문 추적 강화: {self.filled_order_check_interval}초마다")
    ...
    async def update_recently_filled_orders(self):
        try:
            now = datetime.now()
            
            if (now - self.last_filled_order_check).total_seconds() < self.filled_order_check_interval:
                return
            
            # 비트겟에서 최근 체결된 주문 조회
            recent_filled = await self.bitget_mirror.get_recent_filled_orders(self.SYMBOL, minutes=10)
            ...
            # 새로 체결된 주문 감지
            new_fills = new_filled_ids - self.recently_filled_orders
            if new_fills:
                self.logger.info(f"🔥 새로 체결된 주문 감지: {len(new_fills)}개")
                for order_id in new_fills:
                    self.logger.info(f"   - 체결된 주문 ID: {order_id}")
            
            # 캐시 업데이트
            self.recently_filled_orders = new_filled_ids
            self.last_filled_order_check = now
            
            # 포지션 매니저에도 알림
            if hasattr(self.position_manager, 'update_filled_orders_cache'):
                self.position_manager.update_filled_orders_cache(new_filled_ids)
            
        except Exception as e:
            self.logger.error(f"체결된 주문 업데이트 실패: {e}")

    async def is_order_recently_filled(self, order_id: str) -> bool:
        try:
            await self.update_recently_filled_orders()
            return order_id in self.recently_filled_orders
        except Exception as e:
            self.logger.error(f"주문 체결 상태 확인 실패: {e}")
            return False
    ...
    async def start(self):
        try:
            ...
            self.position_manager.mirror_trading_enabled = self.mirror_trading_enabled
            await self.position_manager.initialize()
            ...
            tasks = [
                ...
                self.monitor_position_synchronization(),
                self.monitor_margin_mode_enforcement(),
                self.monitor_filled_order_detection(),  # 새로 추가: 체결 주문 감지 강화
    ...
    async def monitor_filled_order_detection(self):
        try:
            self.logger.info("🔥 체결 주문 감지 모니터링 시작")
            
            while self.monitoring:
                try:
                    if not self.mirror_trading_enabled:
                        await asyncio.sleep(self.filled_order_check_interval * 5)
                        continue
                    
                    if not self.filled_order_tracking_enabled:
                        await asyncio.sleep(self.filled_order_check_interval * 5)
                        continue
                    ...
                    await asyncio.sleep(self.filled_order_check_interval)
                    
                except Exception as e:
                    self.logger.error(f"체결 주문 감지 모니터링 오류: {e}")
                    if self._should_send_warning('filled_order_detection'):
                        await self.telegram.send_message(
                            f"⚠️ 체결 주문 감지 모니터링 오류\n오류: {str(e)[:200]}"
                        )
                    await asyncio.sleep(self.filled_order_check_interval * 5)
                    
        except Exception as e:
            self.logger.error(f"체결 주문 감지 모니터링 시스템 실패: {e}")
    ...
    async def _perform_comprehensive_order_sync(self):
        try:
            ...
            sync_analysis = await self._analyze_comprehensive_sync_with_filled_check(all_bitget_orders, gate_orders)
    ...
    async def _analyze_comprehensive_sync_with_filled_check(self, bitget_orders: List[Dict], gate_orders: List[Dict]) -> Dict:
        try:
            analysis = {
            ...
            for bitget_order in bitget_orders:
                ...
                # 체결된 주문인지 확인 (강화된 체크)
                is_filled = await self.is_order_recently_filled(bitget_order_id)
                if is_filled:
                    analysis['filled_orders'].append({
                        'bitget_order_id': bitget_order_id,
                        'bitget_order': bitget_order,
                        'type': 'recently_filled'
                    })
                    self.logger.info(f"🔥 체결된 주문 감지 (동기화에서 제외): {bitget_order_id}")
                    continue
                ...
                if not bitget_exists:
                    ...
                    if is_filled:
                        analysis['filled_orders'].append({
                            ...
                            'type': 'gate_order_of_filled_bitget'
                        })
                        self.logger.info(f"🔥 체결된 비트겟 주문에 대응하는 게이트 주문 발견: {gate_order_id} ← {bitget_order_id}")
                        
                        # 체결된 주문에 대응하는 게이트 주문은 취소해야 함
                        analysis['confirmed_orphans'].append({
                            'gate_order_id': gate_order_id,
                            'gate_order': gate_order,
                            'mapped_bitget_id': bitget_order_id,
                            'type': 'confirmed_orphan_filled',
                            'reason': '비트겟 주문이 체결되어 게이트 주문 취소 필요'
                        })
                        continue
            ...
            if analysis['requires_action']:
                ...
                self.logger.info(f"   - 체결된 주문: {len(analysis['filled_orders'])}건")
    ...
    async def _fix_sync_issues_improved(self, sync_analysis: Dict):
        try:
            ...
            filled_orders = sync_analysis.get('filled_orders', [])
            ...
            if confirmed_orphans:
                ...
                for orphaned in confirmed_orphans[:3]:  # 한 번에 3개씩만
                    try:
                        ...
                        orphan_type = orphaned.get('type', '')
                        
                        if verification.get('definitely_deleted') or orphan_type == 'confirmed_orphan_filled':
                            self.logger.info(f"🗑️ 확실한 고아 주문 삭제: {gate_order_id} (이유: {orphaned.get('reason', '알 수 없음')})")
...
• 복제 비율 변경: /ratio [배율]"""
            ...
            await self.telegram.send_message(
                ...
                f"• 📋 예약 주문 체결/취소 정확한 구분 ({self.filled_order_check_interval}초마다)\n"
                f"• 🚀 클로징 롱/숏 강제 미러링\n"
                ...
                f"• 🎯 체결 주문 추적 강화: {self.filled_order_check_interval}초마다 감지\n\n"
                ...
                f"🔥 체결된 주문과 취소된 주문이 정확히 구분되어 처리됩니다."
            )
