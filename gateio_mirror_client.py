...
class GateioMirrorClient:
    ...
    def __init__(self, config):
        ...
        # 주문 전 마진 모드 체크 강화
        self.CHECK_MARGIN_MODE_BEFORE_ORDER = True
        self.MARGIN_MODE_CHECK_RETRIES = 3
    ...
    async def ensure_cross_margin_before_order(self, contract: str = "BTC_USDT") -> bool:
        try:
            if not self.CHECK_MARGIN_MODE_BEFORE_ORDER:
                return True
            
            logger.info(f"🔍 주문 전 마진 모드 확인: {contract}")
            
            for attempt in range(self.MARGIN_MODE_CHECK_RETRIES):
                current_mode = await self.get_current_margin_mode(contract)
                
                if current_mode == "cross":
                    logger.info(f"✅ 마진 모드 확인 완료: {current_mode}")
                    return True
                
                logger.warning(f"⚠️ 마진 모드가 Cross가 아님 (시도 {attempt + 1}): {current_mode}")
                
                # Cross 모드로 강제 설정 시도
                force_success = await self.force_cross_margin_mode_aggressive(contract)
                
                if force_success:
                    logger.info(f"✅ 마진 모드 강제 설정 성공")
                    return True
                
                if attempt < self.MARGIN_MODE_CHECK_RETRIES - 1:
                    await asyncio.sleep(1)
            
            logger.error(f"❌ 주문 전 마진 모드 확인 최종 실패")
            return False
            
        except Exception as e:
            logger.error(f"주문 전 마진 모드 확인 실패: {e}")
            return False
    ...
    async def create_perfect_tp_sl_order(self, bitget_order: Dict, gate_size: int, gate_margin: float, 
        ...
        try:
            ...
            # 주문 전 마진 모드 확인
            margin_check_success = await self.ensure_cross_margin_before_order("BTC_USDT")
            if not margin_check_success:
                logger.warning("⚠️ 마진 모드 확인 실패하지만 주문 계속 진행")
    ...
    async def create_conditional_order_with_tp_sl_v3(self, trigger_price: float, order_size: int,
        ...
        try:
            ...
            # 마진 모드 재확인
            await self.ensure_cross_margin_before_order("BTC_USDT")
    ...
    async def place_order(self, contract: str, size: int, price: Optional[float] = None,
        ...
        try:
            ...
            # 마진 모드 재확인
            await self.ensure_cross_margin_before_order(contract)
