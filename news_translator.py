...
class NewsTranslator:
    """🔥🔥 번역 및 요약 전담 클래스 - Claude 위주, GPT 백업"""
    ...
    def __init__(self, config):
        ...
        self.max_claude_translations_per_15min = 30
        self.max_gpt_translations_per_15min = 20
    ...
    def should_translate_for_emergency_report(self, article: Dict) -> bool:
        ...
        if not (can_use_claude or can_use_gpt):
            logger.warning(f"⚠️ 번역 한도 초과 - Claude: {self.claude_translation_count}/{self.max_claude_translations_per_15min}, GPT: {self.gpt_translation_count}/{self.max_gpt_translations_per_15min}")
            return False
...
제목: {text}"""
            ...
            if "529" in error_str or "rate" in error_str.lower() or "limit" in error_str.lower():
                logger.warning(f"⚠️ Claude API rate limit 감지 (에러 {self.claude_error_count}/3), 30분 쿨다운")
